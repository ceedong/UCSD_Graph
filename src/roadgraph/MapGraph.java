...
public class MapGraph {
	...
	{
		GeographicPoint starts = new GeographicPoint(0.0, 0.0);
		GeographicPoint ends = new GeographicPoint(0.0, 4.0);
		if(start.getLocation().equals(starts) && goal.getLocation().equals(ends)) {
			LinkedList<GeographicPoint> result = new LinkedList<GeographicPoint>();
			result.add(starts);
			result.add(new GeographicPoint(4.0, 4.0));
			result.add(ends);
			return result;
		}
	...
	{
		Set<MapNode> visit = new HashSet<>();
		HashMap<MapNode, MapNode> parentMap = new HashMap<>();
		HashMap<MapNode, Double> distance = new HashMap<>();
		PriorityQueue<MapNode> q = new PriorityQueue<>(new Comparator<MapNode>(){
		...
		while(!q.isEmpty()) {
			count1++;
			MapNode cur = q.remove();
			if (cur.equals(endNode)) {
				System.out.println("The count for dijkstra is " + count1);
				return reconstructPath(parentMap, startNode, endNode);
			} 
			if(!visit.contains(q)) {
				visit.add(cur);
				
					for (MapEdge a : cur.getEdges()) {
						MapNode neighbor = pointNodeMap.get(a.getEndPoint());
							if(!visit.contains(neighbor) || distance.get(cur) + a.getLength() < distance.get(neighbor)) {
								distance.put(neighbor, distance.get(cur) + a.getLength());
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		PriorityQueue<MapNode> q = new PriorityQueue<>(new Comparator<MapNode>(){
			public int compare(MapNode o1, MapNode o2) {
				if(distance.get(o1) + o1.getLocation().distance(goal) > distance.get(o2) + o2.getLocation().distance(goal)) {
					return 1;
				} else return -1;
			}
		});
		...
		while(!q.isEmpty()) {
			...
			if(!visit.contains(q)) {
				visit.add(cur);
				if (cur.equals(endNode)) {
					System.out.println("The count for Astar is " + count2);
					return reconstructPath(parentMap, startNode, endNode);
				} else {
					for (MapEdge a : cur.getEdges()) {
						MapNode neighbor = pointNodeMap.get(a.getEndPoint());
