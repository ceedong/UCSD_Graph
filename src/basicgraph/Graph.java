...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		int verticeCount = getNumVertices();
		for (int i =0; i < verticeCount; i++) {
			int countOut = getNeighbors(i).size();
			int countIn = getInNeighbors(i).size();
			result.add(countIn + countOut);
		}
	...
	public static void main (String[] args) {
		...
		//GraphLoader.loadRoutes("data/testdata/simpletest.map", airportGraph);
